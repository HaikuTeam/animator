...
const cli = new Nib({
  name: "haiku",
  version: "0.0.0",
  description: "The Haiku CLI — developer utilities for automating Haiku actions and performing local and server-enabled actions without requiring the desktop app.",
      ...
      args: [
        {
          name: "project-name",
          required: false,
          usage: "Specifies the name of the project to delete (case-sensitive.)  If this isn't provided, the action will be interactive.",
        },
      ],
    },
    {
      ...
      args: [
        {
          name: "project-name",
          required: false,
          usage: "Specifies the name of the project to update as a dependency.  Case-sensitive.  If not provided, will update all detected Haiku projects.",
        },
        {
          name: "version",
          required: false,
          usage: "Specifies the version to update specified dependency to.  If not provided, will update to the latest available version.",
        },
      ],
      action: doUpdate,
    ...
    hasbin("npm", (result) => {
      if (result) {
        ...
        if (fs.existsSync(process.cwd() + "/package.json")) {
          ...
          inkstone.organization.list(token, (listErr, orgs) => {
            ...
            inkstone.project.getByName(token, projectName, (getByNameErr, projectAndCredentials) => {
              ...
              try {
                ...
                context.writeLine(chalk.red("npm install failed.") + " Your Haiku packages have been injected into package.json, but npm install failed.  Please try again.")
                process.exit(1)
              }
          ...
          context.writeLine("You can use " + chalk.bold("haiku clone ProjectName [/Optional/Destination]") + " to clone the project's git repo directly.")
          process.exit(1)
        }
      } else {
        context.writeLine(chalk.red("npm was not found on this machine. ") + " We recommend installing it with nvm: https://github.com/creationix/nvm")
        process.exit(1)
      }
    })
    ...
    if (context.flags.organizations) {
      ...
      inkstone.project.list(token, (err, projects) => {
        if (!projects || projects.length === 0) {
          ...
          context.writeLine("(To work with one, call " + chalk.bold("haiku clone project_name") + " or " + chalk.bold("haiku install project_name"))
  ...
  hasbin("npm", (result) => {
    if (result) {
      try {
        ...
        context.writeLine(chalk.red("npm update failed.") + " This may be a configuration issue with npm.  Try running npm install and then running haiku update again.")
        process.exit(1)
      }
    } else {
      context.writeLine(chalk.red("npm was not found on this machine. ") + " We recommend installing it with nvm: https://github.com/creationix/nvm")
