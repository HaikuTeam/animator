...
function tokenize(source) {
  ...
  while (chunk.length > 0) {
    for (let i = 0; i < REGEXPS.length; i++) {
      ...
      let match = regexp.re.exec(chunk)
      if (match) {
        let value = match[0]
...
export default function functionToRFO(fn) {
  ...
  let pidx1 = str.indexOf("(")
  let pidx2 = str.indexOf(")")
  let prefix = str.slice(0, pidx1)
  let signature = str.slice(pidx1, pidx2 + 1)
  let suffix = str.slice(pidx2 + 1, str.length)
  let body = suffix.slice(suffix.indexOf("{") + 1, suffix.length - 1).trim() // Don't include braces or padding
  let type = suffix.match(/^\s*=>\s*{/)
    ? "ArrowFunctionExpression"
    : "FunctionExpression"
  let name = nth(2, "identifier", tokenize(prefix)).value
  let params = signatureToParams(signature)
