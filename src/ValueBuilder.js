...
SUMMONABLES['$timeline_name'] = {
  summon: function (timelineName, selector, propertyName, keyframeMs, keyframeCluster, hostInstance, inputValues, eventsFired) {
    return timelineName
  }
}
...
SUMMONABLES['$frame'] = {
  summon: function (timelineName, selector, propertyName, keyframeMs, keyframeCluster, hostInstance, inputValues, eventsFired) {
    var timeline = hostInstance.getTimeline && hostInstance.getTimeline(timelineName)
...
ValueBuilder.prototype._clearCachedClusters = function _clearCachedClusters (timelineName, selector) {
  if (this._parsees[timelineName]) this._parsees[timelineName][selector] = {}
  return this
}

ValueBuilder.prototype.evaluate = function _evaluate (fn, timelineName, selector, propertyName, keyframeMs, keyframeCluster, hostInstance, inputValues, eventsFired, inputsChanged) {
  if (!fn.specification) {
  ...
  if (fn.specification === true) { // This function is of an unknown kind, so just evaluate it normally without magic dependency injection
    evaluation = fn.call(hostInstance, inputValues)
  } else if (!Array.isArray(fn.specification.params)) { // If for some reason we got a non-array params, just evaluate
    evaluation = fn.call(hostInstance, inputValues)
  } else if (fn.specification.params.length < 1) { // If for some reason we got 0 params, just evaluate it
    evaluation = fn.call(hostInstance, inputValues)
  } else {
    var summons = fn.specification.params[0] // For now, ignore all subsequent arguments

    if (!summons || typeof summons !== 'object') { // If the summon isn't in the destructured object format, just evaluate it
      evaluation = fn.call(hostInstance, inputValues)
    } else {
      var summonees = this.summonSummonables(summons, timelineName, selector, propertyName, keyframeMs, keyframeCluster, hostInstance, inputValues, eventsFired, inputsChanged)

      if (_areSummoneesDifferent(fn.specification.summonees, summonees)) { // If the summonees are different, evaluate it and cache the newcomers
        fn.specification.summonees = summonees
        evaluation = fn.call(hostInstance, summonees)
      } else { // Since nothing is different, return the previous evaluation
...
ValueBuilder.prototype.summonSummonables = function _summonSummonables (summons, timelineName, selector, propertyName, keyframeMs, keyframeCluster, hostInstance, inputValues, eventsFired, inputsChanged) {
  var summonables = {}
  ...
  for (var key in summons) {
    ...
    if (SUMMONABLES[key.toLowerCase()]) {
      ...
      summonables[key] = SUMMONABLES[key].summon(timelineName, selector, propertyName, keyframeMs, keyframeCluster, hostInstance, inputValues, eventsFired, inputsChanged)
      continue
    }
...
ValueBuilder.prototype.fetchParsedValueCluster = function _fetchParsedValueCluster (timelineName, selector, outputName, cluster, hostInstance, inputValues, eventsFired, inputsChanged, isPatchOperation, skipCache) {
  ...
  if (!this._parsees[timelineName][selector][outputName]) this._parsees[timelineName][selector][outputName] = {}
  ...
  for (var ms in cluster) {
    ...
    if (isFunction(descriptor.value)) {
      ...
      if (descriptor.curve) this._parsees[timelineName][selector][outputName][ms].curve = descriptor.curve
      ...
      var functionReturnValue = this.evaluate(descriptor.value, timelineName, selector, outputName, ms, cluster, hostInstance, inputValues, eventsFired, inputsChanged)
...
ValueBuilder.prototype.generateFinalValueFromParsedValue = function _generateFinalValueFromParsedValue (timelineName, selector, outputName, computedValue) {
 ...
 */
ValueBuilder.prototype.build = function _build (out, timelineName, timelineTime, timelinesObject, isPatchOperation, inputValues, eventsFired, inputsChanged) {
  ...
  for (var selector in overrides) {
    ...
    for (var propertyName in propertiesGroup) {
      ...
      var finalValue = this.grabValue(timelineName, selector, propertyName, propertiesGroup, timelineTime, haikuComponent, inputValues, eventsFired, inputsChanged, isPatchOperation)
      ...
      if (!isPatchOperation || this.didChangeValue(timelineName, selector, propertyName, finalValue)) {
        if (!out[selector]) out[selector] = {}
        if (out[selector][propertyName] === undefined) out[selector][propertyName] = finalValue
        else out[selector][propertyName] = BasicUtils.mergeValue(out[selector][propertyName], finalValue)
      }
    }
  }
  return out
 ...
 */
ValueBuilder.prototype.grabValue = function _grabValue (timelineName, selector, propertyName, propertiesGroup, timelineTime, haikuPlayer, inputValues, eventsFired, inputsChanged, isPatchOperation, skipCache) {
  var parsedValueCluster = this.fetchParsedValueCluster(timelineName, selector, propertyName, propertiesGroup[propertyName], haikuPlayer, inputValues, eventsFired, inputsChanged, isPatchOperation, skipCache)
  ...
  if (isPatchOperation && !skipCache) {
    computedValueForTime = Transitions.calculateValueAndReturnUndefinedIfNotWorthwhile(parsedValueCluster, timelineTime)
  } else {
    computedValueForTime = Transitions.calculateValue(parsedValueCluster, timelineTime)
  }
  ...
  var finalValue = this.generateFinalValueFromParsedValue(timelineName, selector, propertyName, computedValueForTime)
