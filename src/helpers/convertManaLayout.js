...
function determineSizingProp (sizeAxis, attributeValue) {
  ...
  switch (parsedValue.unit) {
    case '%': return { name: 'sizeProportional', value: parsedValue.value / 100, mode: Layout3D.SIZE_PROPORTIONAL }
    case 'px': return { name: 'sizeAbsolute', value: parsedValue.value, mode: Layout3D.SIZE_ABSOLUTE }
    case '': return { name: 'sizeAbsolute', value: parsedValue.value, mode: Layout3D.SIZE_ABSOLUTE }
...
module.exports = function convertManaLayout (mana) {
  visitManaTree(ROOT_LOCATOR, mana, function _visitor (name, attributes, children, node, locator, parent, index) {
    ...
    if (attributes.style && typeof attributes.style === 'object') {
      ...
      if (attributes.style.width !== undefined && attributes.style.width !== null) {
      ...
      if (attributes.style.height !== undefined && attributes.style.height !== null) {
    ...
    if (attributes.transform !== undefined && attributes.transform !== null) {
      ...
      for (var transformAttributeName in transformAttributes) {
        ...
        if (!TRANSFORM_COMPONENT_WHITELIST[transformAttributeName]) {
          console.warn('Skipping transform attribute ' + transformAttributeName + ' because it is not yet supported')
          continue
        }
