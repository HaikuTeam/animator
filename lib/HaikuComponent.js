...
HaikuComponent.prototype._fetchTimelines = function _fetchTimelines() {
    ...
    for (var i = 0; i < names.length; i++) {
        ...
        if (!existing) {
            this._timelineInstances[name_1] = new HaikuTimeline(this, name_1, descriptor, this.config.options);
        }
    }
...
function _applyPropertyToElement(element, name, value, context, component) {
    ...
    if (vanityHandlers[type] &&
        vanityHandlers[type][name]) {
        vanityHandlers[type][name](name, element, value, context, component);
    }
    else {
