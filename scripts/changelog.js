...
const CHANGELOG_JSON = path.join(ROOT, 'changelog/changelog.json');
const CHANGELOG_MD = path.join(ROOT, 'changelog/CHANGELOG.md');

const monoPkg = fse.readJsonSync(path.join(ROOT, 'package.json'));
const monoVersion = monoPkg.version;

const changelog = fse.readJsonSync(CHANGELOG_JSON);
...
async.eachSeries(allPackages, (pack, next) => {
  ...
  const publicLines = lines.filter((line) => line.message && line.sha && PUBLIC_CHANGELOG_LABELS[line.label]);

  // Only worry about the most recent 50 commits per project (speed things up not to go thru them all?)
  publicLines.splice(50);

  const notYetAddedLines = publicLines.filter((line) => !isLineAlreadyInChangelog(line));
