...
tape('envoy:multi-server', async (t) => {
    ...
    var s1 = new EnvoyServer({ logger: new EnvoyLogger("error") })
    s1 = await s1.ready()

    var s2 = new EnvoyServer({ logger: new EnvoyLogger("error") })
    s2 = await s2.ready()

    var s3 = new EnvoyServer({ logger: new EnvoyLogger("error") })
    s3 = await s3.ready()
