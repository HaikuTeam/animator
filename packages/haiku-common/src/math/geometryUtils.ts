...
// Taken from: https://github.com/Pomax/bezierjs/blob/9d62b24808848dc5acbed0fdbf7b440e6efca4d3/lib/utils.js#L642
export const bezierInflections = (points: Vec2[]) => {
  ...
  if (isApproximately(v1, 0)) {
    if (!isApproximately(v2, 0)) {
      const t = -v3 / v2;
      if (0 <= t && t <= 1) { return [t]; }
    }
    return [];
  ...
  if (isApproximately(d2, 0)) { return []; }
...
// Taken from: https://github.com/Pomax/bezierjs/blob/9d62b24808848dc5acbed0fdbf7b440e6efca4d3/lib/utils.js#L201
export const isApproximately = (a: number, b: number, precision?: number) => {
  const epsilon = 0.000001;
  return Math.abs(a - b) <= (precision || epsilon);
};

// Taken from: https://github.com/Pomax/bezierjs/blob/9d62b24808848dc5acbed0fdbf7b440e6efca4d3/lib/utils.js#L463
export const align = (points: Vec2[], line: { p1: Vec2, p2: Vec2 }) => {
  const tx = line.p1.x;
  const ty = line.p1.y;
  const a = -Math.atan2(line.p2.y - ty, line.p2.x - tx);

  return points.map((v: Vec2) => {
    return {
...
export const linearMap = (inMin: number, inMax: number, outMin: number, outMax: number, v: number) => {
