...
tape('master.03.git', (t) => {
  ...
  TestHelpers.setup(function(folder, creator, glass, timeline, metadata, teardown) {
    ...
    async.series([
      ...
      function (cb) {
        t.equal(master._git._gitUndoables.length, 2, 'two git undoables')
        return cb()
      },
      function (cb) {
        fse.writeFileSync(path.join(folder, 'Hello.svg'), '<svg><rect x="0" y="0" stroke="1" fill="black"></rect></svg>')
        setTimeout(() => {
          t.equal(master._git._gitUndoables.length, 3, 'three git undoables')
          return master.instantiateComponent({ params: ['Hello.svg', {}] }, cb)
        }, 2000)
      },
      function (cb) {
        setTimeout(() => {
          t.equal(master._git._gitUndoables.length, 4, 'four git undoables')
          return cb()
        }, 2000)
      }
    ], (err) => {
