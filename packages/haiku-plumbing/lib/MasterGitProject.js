...
var MasterGitProject = function (_EventEmitter) {
  ...
  _createClass(MasterGitProject, [{
    ...
    value: function teardown(cb) {
      ...
      return this.waitUntilNoFurtherRequestsArePending(function () {
        _this3._requestWorkerStopped = true;
        return cb();
      });
    }
  }, {
    ...
    value: function runActionSequence(seq, projectOptions, cb) {
      ...
      return _async2.default.eachSeries(seq, function (method, next) {
        return _this4.fetchFolderState('action-sequence=' + method, projectOptions, function (err) {
          ...
          return _this4[method](next);
        });
      }, function (err) {
        if (err) return cb(err);
        // Recipients of this response also depend on the folderState being up to date
        return _this4.fetchFolderState('action-sequence=done', projectOptions, cb);
      });
    }
  }, {
    key: 'getFolderState',
    ...
    value: function fetchFolderState(who, projectOptions, cb) {
      ...
      return _async2.default.series([function (cb) {
        return _this5.safeHasAnyHeadCommitForCurrentBranch(function (hasHeadCommit) {
          _this5._folderState.hasHeadCommit = hasHeadCommit;
          return cb();
        });
      }, function (cb) {
        return Git.referenceNameToId(_this5.folder, 'HEAD', function (_err, headCommitId) {
          _this5._folderState.headCommitId = headCommitId;
          return cb();
        });
      }, function (cb) {
        return _this5.safeFetchProjectGitRemoteInfo(function (remoteProjectDescriptor) {
          _this5._folderState.remoteProjectDescriptor = remoteProjectDescriptor;
          _this5._folderState.isCodeCommitReady = !!(_this5._projectInfo.projectName && remoteProjectDescriptor);
          return cb();
        });
      }, function (cb) {
        return _this5.safeListLocallyDeclaredRemotes(function (gitRemotesList) {
          _this5._folderState.gitRemotesList = gitRemotesList;
          return cb();
        });
      }, function (cb) {
        return _checkIsOnline(function (isOnline) {
          _this5._folderState.isOnline = isOnline;
          return cb();
        });
      }, function (cb) {
        return _this5.safeGitStatus({ log: false }, function (gitStatuses) {
          _this5._folderState.doesGitHaveChanges = !!(gitStatuses && Object.keys(gitStatuses).length > 0);
          _this5._folderState.isGitInitialized = _haikuFsExtra2.default.existsSync(_path2.default.join(_this5.folder, '.git'));
          return cb();
        });
      }, function (cb) {
        _this5._folderState.folderEntries = _haikuFsExtra2.default.readdirSync(_this5.folder);
        _this5._folderState.folder = _this5.folder;
        _this5._folderState.projectName = _this5._projectInfo.projectName;
        _this5._folderState.branchName = _this5._projectInfo.branchName;
        _this5._folderState.haikuUsername = _this5._projectInfo.haikuUsername;
        _this5._folderState.haikuPassword = _this5._projectInfo.haikuPassword;
        _this5._folderState.gitUndoables = _this5._gitUndoables;
        _this5._folderState.gitRedoables = _this5._gitRedoables;
        return cb();
      }, function (cb) {
        var packageJsonExists = _haikuFsExtra2.default.existsSync(_path2.default.join(_this5.folder, 'package.json'));
        if (!packageJsonExists) return cb();
        var packageJsonObj = _haikuFsExtra2.default.readJsonSync(_path2.default.join(_this5.folder, 'package.json'), { throws: false });
        if (!packageJsonObj) return cb();
        _this5._folderState.semverVersion = packageJsonObj.version;
        _this5._folderState.playerVersion = packageJsonObj.dependencies && packageJsonObj.dependencies['@haiku/player'];
        return cb();
      }], function (err) {
        if (err) return cb(err);
        _LoggerInstance2.default.info('[master-git] folder state fetch (' + who + ') done');
        return cb(null, _this5._folderState, previousState);
      });
    }
  }, {
    ...
    value: function safeFetchProjectGitRemoteInfo(cb) {
      ...
      return Inkstone.project.getByName(authToken, this._projectInfo.projectName, function (err, projectAndCredentials, httpResp) {
        ...
        return cb({ // eslint-disable-line
          projectName: _this6._projectInfo.projectName,
    ...
    value: function waitUntilNoFurtherChangesAreAwaitingCommit(cb) {
      ...
      return setTimeout(function () {
        return _this7.waitUntilNoFurtherChangesAreAwaitingCommit(cb);
    ...
    key: 'waitUntilNoFurtherRequestsArePending',
    value: function waitUntilNoFurtherRequestsArePending(cb) {
      var _this8 = this;
      ...
      return setTimeout(function () {
        return _this8.waitUntilNoFurtherRequestsArePending(cb);
      }, AWAIT_COMMIT_INTERVAL);
    }
    ...
    value: function bumpSemverAppropriately(cb) {
      ...
      return Git.listTags(this.folder, function (err, tags) {
        ...
        var pkgTag = _haikuFsExtra2.default.readJsonSync(_path2.default.join(_this9.folder, 'package.json')).version;
        ...
        return ProjectFolder.semverBumpPackageJson(_this9.folder, nextTag, function (err) {
          if (err) return cb(err);
          ...
          _this9.emit('semver-bumped', nextTag, function () {
      ...
      return Git.createTag(this.folder, this._folderState.semverVersion, this._folderState.commitId, this._folderState.semverVersion, function (err) {
        if (err) {
          ...
          if (err.message && err.message.match(/Tag already exists/i)) {
            _LoggerInstance2.default.info('[master-git] git tag ' + _this10._folderState.semverVersion + ' already exists; trying to bump it');

            return _this10.bumpSemverAppropriately(function (err, incTag) {
              if (err) return cb(err);

              _this10._folderState.semverVersion = incTag;
        ...
        _LoggerInstance2.default.sacred('[master-git] git tagged: ' + _this10._folderState.semverVersion);
    ...
    value: function retryCloudSaveSetup(cb) {
      ...
      return this.ensureAllRemotes(function (err) {
        if (err) {
          return _this11.cloudSaveDisabled(cb);
        }

        return _this11.fetchFolderState('cloud-setup', {}, function (err) {
          if (err) {
            return _this11.cloudSaveDisabled(cb);
          }

          if (!_this11._folderState.isGitInitialized) {
            return _this11.cloudSaveDisabled(cb);
          }
    ...
    value: function pushToRemote(cb) {
      ...
      return Git.pushProject(this.folder, this._folderState.projectName, GitRemoteUrl, CodeCommitHttpsUsername, CodeCommitHttpsPassword, function (err) {
        if (err) return cb(err);
        return _this12.pushTag(GitRemoteUrl, CodeCommitHttpsUsername, CodeCommitHttpsPassword, cb);
      });
    }
  }, {
    key: 'initializeGit',
    ...
    value: function moveContentsToTemp(cb) {
      ...
      return _tmp2.default.dir({ unsafeCleanup: true }, function (err, tmpDir, tmpDirCleanupFn) {
        ...
        _this13._folderState.tmpDir = tmpDir;

        _LoggerInstance2.default.info('[master-git] temp dir is', _this13._folderState.tmpDir);

        _this13._folderState.tmpDirCleanupFn = tmpDirCleanupFn;
        ...
        if (_this13._folderState.folderEntries.length < 1) {
          _LoggerInstance2.default.info('[master-git] folder had no initial content; skipping temp folder step');

          return cb();
        }

        _LoggerInstance2.default.info('[master-git] copying contents from', _this13.folder, 'to temp dir', _this13._folderState.tmpDir);

        return _haikuFsExtra2.default.copy(_this13.folder, _this13._folderState.tmpDir, function (err) {
          if (err) return cb(err);

          _LoggerInstance2.default.info('[master-git] emptying original dir', _this13.folder);

          // Folder must be empty for a Git clone to take place
          return _haikuFsExtra2.default.emptyDir(_this13.folder, function (err) {
            if (err) return cb(err);
            return cb();
          });
        });
    ...
    value: function cloneRemoteIntoFolder(cb) {
      ...
      return Git.cloneRepo(GitRemoteUrl, CodeCommitHttpsUsername, CodeCommitHttpsPassword, this.folder, function (err) {
        if (err) {
          ...
          if (_this14._folderState.cloneAttempts < MAX_CLONE_ATTEMPTS) {
            _LoggerInstance2.default.info('[master-git] retrying clone after a brief delay...');

            return setTimeout(function () {
              return _this14.cloneRemoteIntoFolder(cb);
            }, CLONE_RETRY_DELAY);
          }
        ...
        return _this14.ensureAllRemotes(function (err) {
          if (err) return cb(err);
          return cb();
        });
      });
    ...
    value: function ensureAllRemotes(cb) {
      ...
      return this.ensureLocalRemote(function (err) {
        if (err) return cb(err);
        return _this15.ensureRemoteRefs(function (err) {
          if (err) return cb(err);
          return cb();
        });
      });
    ...
    value: function ensureRemoteRefs(cb) {
      ...
      return Git.open(this.folder, function (err, repository) {
        ...
        return _haikuFsExtra2.default.outputFile(_path2.default.join(_this16.folder, 'README.md'), '', function (err) {
          if (err) return cb(err);

          _LoggerInstance2.default.info('[master-git] remote refs: making base commit');

          return Git.addAllPathsToIndex(_this16.folder, function (err, oid) {
            if (err) {
              return cb(err);
            }

            return Git.buildCommit(_this16.folder, _this16._folderState.haikuUsername, null, 'Base commit ' + COMMIT_SUFFIX, oid, null, null, function (err, commitId) {
              if (err) return cb(err);
              ...
              return repository.createBranch(branchName, commitId.toString()).then(function () {
                return Git.lookupRemote(_this16.folder, _this16._folderState.projectName, function (err, mainRemote) {
                  if (err) return cb(err);

                  var remoteRefspecs = [refSpecToPush];
                  var remoteCreds = Git.buildRemoteOptions(_this16._folderState.remoteProjectDescriptor.CodeCommitHttpsUsername, _this16._folderState.remoteProjectDescriptor.CodeCommitHttpsPassword);
    ...
    value: function copyContentsFromTemp(cb) {
      ...
      return _haikuFsExtra2.default.copy(this._folderState.tmpDir, this.folder, function (err) {
        if (err) return cb(err);
        _LoggerInstance2.default.info('[master-git] cleaning up temp dir', _this17._folderState.tmpDir);
        _this17._folderState.tmpDirCleanupFn();
        return cb();
      });
    }
  }, {
    ...
    value: function pullRemote(cb) {
      ...
      return Git.fetchProject(this.folder, this._folderState.projectName, GitRemoteUrl, CodeCommitHttpsUsername, CodeCommitHttpsPassword, function (err) {
        ...
        return Git.getCurrentBranchName(_this18.folder, function (err, partialBranchName) {
          if (err) return cb(err);
          _LoggerInstance2.default.info('[master-git] current branch is \'' + partialBranchName + '\'');

          return Git.mergeProject(_this18.folder, _this18._folderState.projectName, partialBranchName, _this18._folderState.saveOptions, function (err, didHaveConflicts, shaOrIndex) {
            if (err) return cb(err);
            ...
            return _this18.safeGitStatus({ log: true }, function () {
              _this18._folderState.didHaveConflicts = didHaveConflicts;
              _this18._folderState.mergeCommitId = didHaveConflicts ? null : shaOrIndex.toString();
              return cb();
            });
          });
        });
    ...
    value: function conflictResetOrContinue(cb) {
      ...
      return this.safeGitStatus({ log: true }, function () {
        return Git.cleanAllChanges(_this19.folder, function (err) {
          if (err) return cb(err);
          return Git.hardResetFromSHA(_this19.folder, _this19._folderState.commitId.toString(), function (err) {
            if (err) return cb(err);
            _this19._folderState.wasResetPerformed = true;
            return cb();
          });
        });
      });
    ...
    value: function getExistingShareDataIfSaveIsUnnecessary(cb) {
      ...
      return this.fetchFolderState('get-existing-share-data', {}, function () {
        ...
        if (_this20._folderState.doesGitHaveChanges) {
        ...
        return _this20.getCurrentShareInfo(2000, function (err, shareInfo) {
    ...
    value: function undoActual(undoOptions, done) {
      ...
      return Git.hardResetFromSHA(this.folder, target.commitId.toString(), function (err) {
        ...
        _this21._gitRedoables.push(_this21._gitUndoables.pop());
    ...
    value: function redoActual(redoOptions, done) {
      ...
      return Git.hardResetFromSHA(this.folder, redoable.commitId.toString(), function (err) {
        if (err) {
          ...
          _this22._gitRedoables.push(redoable); // If error, put the 'undone' thing back on the stack since we didn't succeed
          return done(err);
        }

        _this22._gitUndoables.push(redoable);
    ...
    value: function setUndoBaselineIfHeadCommitExists(cb) {
      ...
      return this.fetchFolderState('undo-baseline', {}, function () {
        ...
        if (_this23._folderState.headCommitId) {
          if (_this23._gitUndoables.length < 1) {
            _LoggerInstance2.default.info('[master-git] base commit for session is ' + _this23._folderState.headCommitId.toString());
            _this23._gitUndoables.push({
              commitId: _this23._folderState.headCommitId,
    ...
    value: function commitFileIfChanged(relpath, message, cb) {
      ...
      return this.waitUntilNoFurtherChangesAreAwaitingCommit(function () {
        return _this24.statusForFile(relpath, function (err, status) {
          if (err) return cb(err);
          ...
          if (status.num && status.num > 0) {
            return _this24.commit(relpath, message, cb);
          } else {
            return cb();
          }
        });
    ...
    value: function commitProjectIfChanged(message, cb) {
      ...
      // The call to status is sync, so we add this hook in case pending commits may alter the status
      return this.waitUntilNoFurtherChangesAreAwaitingCommit(function () {
        return _this25.safeGitStatus({ log: true }, function (gitStatuses) {
          ...
          if (doesGitHaveChanges) {
            ...
            return _this25.commit('.', message, cb);
          }
          return cb();
        });
      });
    ...
    value: function commitActual(commitOptions, cb) {
      ...
      return this.fetchFolderState('commit-project', {}, function () {
        return Git.commitProject(_this26.folder, _this26._folderState.haikuUsername, _this26._folderState.hasHeadCommit, finalOptions, addable, function (err, commitId) {
          if (err) {
            return cb(err);
          }

          _this26._folderState.commitId = commitId;
          ...
          var baseUndoable = _this26._gitUndoables.filter(function (undoable) {
          ...
          _LoggerInstance2.default.sacred('[master-git] commit ' + commitId.toString() + ' (base: ' + isBase + ')');
          ...
          _this26._gitUndoables.push({ commitId: commitId, isBase: isBase, message: message });
    ...
    value: function initializeProject(initOptions, done) {
      ...
      return _async2.default.series([function (cb) {
        return _this27.fetchFolderState('initialize-folder', initOptions, function (err) {
          if (err) return cb(err);
          _LoggerInstance2.default.info('[master-git] folder initialization status:', _this27._folderState);
          return cb();
        });
      }, function (cb) {
        var _folderState = _this27._folderState,
        ...
        return _this27.runActionSequence(actionSequence, initOptions, function (err) {
          if (err) return cb(err);
          return cb();
        });
    ...
    value: function saveProject(saveOptions, done) {
      ...
      return _async2.default.series([function (cb) {
        return _this28.waitUntilNoFurtherChangesAreAwaitingCommit(cb);
      }, function (cb) {
        return _this28.fetchFolderState('save-project', saveOptions, function (err) {
          if (err) return cb(err);
          _this28._folderState.semverVersion = saveAccumulator.semverVersion;
          _this28._folderState.saveOptions = saveOptions;
          _LoggerInstance2.default.info('[master-git] pre-save status:', _this28._folderState);
          return cb();
        });
      }, function (cb) {
        _LoggerInstance2.default.info('[master-git] project save: preparing action sequence');

        var _folderState2 = _this28._folderState,
        ...
        return _this28.runActionSequence(actionSequence, saveOptions, cb);
      }, function (cb) {
        ...
        if (_this28._folderState.didHaveConflicts) {
        ...
        return _this28.getCurrentShareInfo(60000 * 2, cb);
      }], function (err, results) {
        if (err) return done(err);
        return done(null, results[results.length - 1]);
      });
