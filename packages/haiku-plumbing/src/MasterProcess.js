...
function isFileSomethingWeDontCareAbout (relpath) {
  if (UNWATCHABLE_RELPATHS[relpath]) return true
  if (UNWATCHABLE_BASENAMES[path.basename(relpath)]) return true
  if (!WATCHABLE_EXTNAMES[path.extname(relpath)]) return true
  return false
}

function excludeIfNotJs (relpath) {
  if (path.extname(relpath) !== '.js') return true
  return isFileSomethingWeDontCareAbout(relpath)
}
...
const methods = {
  ...
  startProject: (message, done) => {
    ...
    return async.series([
      ...
      function (cb) {
        logger.info(`[master] start project: ingesting js files in folder ${folder}`)
        return File.ingestFromFolder(ProcessBase.HAIKU.folder, {
          exclude: excludeIfNotJs
        }, cb)
      },
...
function _createFileWatcherChangeHandler (File, activeComponent) {
  return function (abspath) {
    ...
    if (isFileSomethingWeDontCareAbout(relpath)) {
      return void (0)
    }
    ...
    if (extname === '.sketch') {
      ...
      KNOWN_DESIGNS[relpath] = { relpath, abspath, dtModified: Date.now() }
      return void (0)
    }
    ...
    if (extname === '.svg') {
      if (proc.socket && proc.socket.isOpen()) {
        ...
        proc.socket.request({ type: 'action', method: 'mergeDesign', params: [folder, 'Default', 0, abspath] }, () => {})
      }
      KNOWN_DESIGNS[relpath] = { relpath, abspath, dtModified: Date.now() }
      return void (0)
    }
...
function _createFileWatcherRemoveHandler (File, activeComponent) {
  return function (abspath) {
    ...
    COMMITABLE_CHANGE_LIST.push({ message: `Removed ${relpath}`, relpath, id: _createChangeId() })
...
function decorate (File) {
  function reloadAssetsDirectory () {
    const assets = Asset.assetsToDirectoryStructure(KNOWN_DESIGNS)
    return assets
  }
