...
export function buildProjectContent (_ignoredLegacyArg, projectPath, projectName, projectType = DEFAULT_PROJECT_TYPE, projectOptions, finish) {
  ...
  try {
    ...
    if (organizationName && organizationName !== FALLBACK_ORG_NAME) {
      ...
      // If their project is still using @haiku/player, update it to @haiku.core
      if (packageJson.dependencies['@haiku/player']) {
        packageJson.dependencies['@haiku/core'] = packageJson.dependencies['@haiku/player']
        delete packageJson.dependencies['@haiku/player']
      }

      // And make sure they do have the @haiku/core as a dependency if not already
      if (!packageJson.dependencies['@haiku/core']) {
        packageJson.dependencies['@haiku/core'] = haikuCoreVersion
      }

      logger.info('[project folder] @haiku/core version is', packageJson.dependencies['@haiku/core'])
