...
class LogViewer extends React.Component {
  constructor (props) {
    ...
    this.state = {
      ...
      displayTags: {
        ...
        'timeline:loop': {color: Palette.PURPLE, label: 'LOOP COUNT', display: true},
        'preview:error': {color: Palette.RED, label: 'ERRORS', display: true},
        'console:log': {color: Palette.ROCK, label: 'CONSOLE', display: true},
      },
    };
  ...
  renderLogFromJsonLog (message) {
    if (message.tag in this.state.displayTags) {
      return `<<${message.attachedObject.componentTitle}>> ${message.message}`;
    }
  ...
  renderLogLine (index, key) {
    ...
    return (
      <div style={{lineHeight: '12px'}}><span style={{color: Palette.ROCK}}>{message.timestamp}</span><span
          key={message.key}
      ...
      <div
        ...
        <SplitPanel split="vertical" minSize={150} defaultSize={300}>
          ...
          <div id="logview-right-panel" style={{position: 'relative', overflow: 'auto', width: '100%', height: '100%', backgroundColor: Palette.COAL}} >
            ...
            <div style={{fontWeight: 'bold', fontFamily: 'Fira Mono', paddingLeft: 20, paddingTop: 4, position: 'relative', overflow: 'auto', width: '100%', height: 'calc(100% - 35px)', backgroundColor: Palette.COAL}} >
              <ReactList
                ref="loglist"
