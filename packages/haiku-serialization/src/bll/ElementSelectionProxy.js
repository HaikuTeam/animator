...
class ElementSelectionProxy extends BaseModel {
  constructor (props, opts) {
    ...
    this._proxyProperties = lodash.assign(ElementSelectionProxy.DEFAULT_PROPERTY_VALUES)
  }
  ...
  getBoxPointsTransformed () {
    ...
    const proxyLayoutSpec = {
      ...
      translation: {
        x: this.getPropertyValue('translation.x'),
        y: this.getPropertyValue('translation.y'),
        z: this.getPropertyValue('translation.z')
      },
      rotation: {
        x: this.getPropertyValue('rotation.x'),
        y: this.getPropertyValue('rotation.y'),
        z: this.getPropertyValue('rotation.z'),
        w: 0
      },
      scale: {
        x: this.getPropertyValue('scale.x'),
        y: this.getPropertyValue('scale.y'),
        z: this.getPropertyValue('scale.z')
      },
      ...
      sizeAbsolute: {
        ...
        z: this.getPropertyValue('sizeAbsolute.z')
      }
    }
  ...
  getBoundingClientRect () {
    if (this.doesManageSingleElement()) {
      // Note: This actually queries the element's DOM element
      const elementRect = this.selection[0].getBoundingClientRect()
      return elementRect
    }
  ...
  getElementOrProxyPropertyValue (key) {
    ...
    if (this.doesManageSingleElement()) {
      return this.selection[0].getPropertyValue(key, fallback)
    }
  ...
  move (dx, dy, coordsCurrent, coordsPrevious, lastMouseDownCoord) {
    ...
    this.selection.forEach((element) => {
      const propertyGroupValue = element.computePropertyGroupValueFromGroupDelta(propertyGroupDelta)

      element.component.applyPropertyGroupValue(
        element.getComponentId(),
        element.component.getCurrentTimelineName(),
        element.component.getCurrentTimelineTime(),
        propertyGroupValue,
        element.component.project.getMetadata(),
        () => {} // no-op
      )
    })
  }
  ...
  moveWithShiftDown (dx, dy, mouseCoordsCurrent, mouseCoordsPrevious, lastMouseDownCoord) {
    this.selection.forEach((element) => {
      const propertyGroupForElement = ElementSelectionProxy.computeConstrainedMovePropertyGroup(
  ...
  scaleElements (
    ...
    this.selection.forEach((element) => {
      const propertyGroupDeltaForElement = ElementSelectionProxy.computeScalePropertyGroupDelta(
        element, // targetElement
        this, // contextElement
        dx,
        dy,
        ...
        activationPoint,
        viewportTransform,
        false // doScaleForArtboard
      )

      const propertyGroupValueForElement = element.computePropertyGroupValueFromGroupDelta(propertyGroupDeltaForElement)

      element.component.applyPropertyGroupValue(
        element.getComponentId(),
        element.component.getCurrentTimelineName(),
        element.component.getCurrentTimelineTime(),
        propertyGroupValueForElement,
        element.component.project.getMetadata(),
        () => {}
      )
    })
  ...
  scaleArtboard (
    ...
    const propertyGroupDeltaForElement = ElementSelectionProxy.computeScalePropertyGroupDelta(
      ...
      activationPoint,
      viewportTransform,
      true /* doScaleForArtboard */
    )

    // Don't allow the user to reduce the artboard's scale to nothing
    if (propertyGroupDeltaForElement.width > 5 && propertyGroupDeltaForElement.height > 5) {
      const propertyGroupValueForElement = element.computePropertyGroupValueFromGroupDelta(propertyGroupDeltaForElement)

      element.component.resizeContext(
        element.getComponentId(),
        element.component.getCurrentTimelineName(),
        0, // Frame zero is hardcoded since artboard resizes at different times is confusing
        propertyGroupValueForElement,
        element.component.project.getMetadata(),
        () => {}
      )
  ...
  rotate (dx, dy, coordsCurrent, coordsPrevious, lastMouseDownCoord, activationPoint) {
    ...
    this.selection.forEach((element) => {
      const propertyGroupDeltaForElement = ElementSelectionProxy.computeRotationPropertyGroupDelta(
        element, // targetElement
        this, // contextElement
        dx,
        dy,
  ...
  duplicateAllAndSelectDuplicates (metadata, cb) {
    const duplicates = []
    return async.eachSeries(this.selection, (element, next) => {
      return this.component.pasteThing(element.clip(metadata), {}, metadata, (err, {haikuId}) => {
...
ElementSelectionProxy.computeScalePropertyGroupDelta = (
  ...
  const sizeDeltaCoefficient = (activationPoint.alt ? 2 : 1)

  const isLeft = activationPoint.index === 6 || activationPoint.index === 3 || activationPoint.index === 0
  const isTop = activationPoint.index === 0 || activationPoint.index === 1 || activationPoint.index === 2

  let dxFinal = dx
  let dyFinal = dy

  if (doScaleForArtboard) {
  ...
  if (doScaleForArtboard) {
    const currentSize = {
      x: targetElement.getPropertyValue('sizeAbsolute.x'),
      y: targetElement.getPropertyValue('sizeAbsolute.y'),
      z: targetElement.getPropertyValue('sizeAbsolute.z') || 0
    }
  ...
  const targetWidth = targetElement.getPropertyValue('sizeAbsolute.x')
  const targetHeight = targetElement.getPropertyValue('sizeAbsolute.y')
  const targetScaleX = targetElement.getPropertyValue('scale.x')
  const targetScaleY = targetElement.getPropertyValue('scale.y')
  const targetAbsoluteWidth = targetScaleX * targetWidth
  const targetAbsoluteHeight = targetScaleY * targetHeight
  const targetThetaRadians = targetElement.getPropertyValue('rotation.z')
  ...
  const propertyGroup = {
    'scale.x': deltaScaleVector[0],
    'scale.y': deltaScaleVector[1],
    'translation.x': translationOffset[0],
    'translation.y': translationOffset[1]
  }
...
ElementSelectionProxy.computeRotationPropertyGroupDelta = (
  ...
  const originX = targetElement.getPropertyValue('origin.x')
  const originY = targetElement.getPropertyValue('origin.y')

  const cx = rect.left + ((rect.right - rect.left) * originX)
  const cy = rect.top + ((rect.bottom - rect.top) * originY)
  ...
  if (activationPoint.shift) {
    ...
    theta0 = targetElement.getPropertyValue('rotation.z')
...
ElementSelectionProxy.fromSelection = (selection, query) => {
  ...
  return ElementSelectionProxy.upsert(lodash.assign({ uid, selection }, query))
}
