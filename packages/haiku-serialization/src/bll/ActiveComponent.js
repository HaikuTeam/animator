...
class ActiveComponent extends BaseModel {
  ...
  unselectElementWithinTime (waitTime, componentId, metadata, cb) {
    ...
    if (!element) {
      if (waitTime <= 0) {
        // Is it better to throw here, like we do with selectElement?
        return cb()
      }
  ...
  */
  ...
  createInstance (bytecode, config) {
    ...
    bytecodeFile.setHostInstance(createdHaikuCoreComponent)
    bytecodeFile.hostInstance = createdHaikuCoreComponent
