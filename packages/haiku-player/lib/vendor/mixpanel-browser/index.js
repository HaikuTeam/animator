...
_.cookie = {
    get: function (name) {
        ...
        for (var i = 0; i < ca.length; i++) {
            var c_1 = ca[i];
            while (c_1.charAt(0) == " ") {
                c_1 = c_1.substring(1, c_1.length);
            }
            if (c_1.indexOf(nameEQ) === 0) {
                return decodeURIComponent(c_1.substring(nameEQ.length, c_1.length));
            }
        }
        return null;
    },
    ...
    set_seconds: function (name, value, seconds, cross_subdomain, is_secure) {
        ...
        if (cross_subdomain) {
            var matches = document$1.location.hostname.match(/[a-z0-9][a-z0-9\-]+\.[a-z\.]{2,6}$/i), domain = matches ? matches[0] : "";
    ...
    function getElementsBySelector(selector) {
        ...
        var _loop_1 = function () {
            ...
            if (token_match) {
                ...
                var attrName_1 = token_match[2];
                var attrOperator = token_match[3];
                var attrValue_1 = token_match[4];
                if (!tagName) {
                    tagName = "*";
                }
                found = [];
                ...
                currentContext = [];
                currentContextIndex = 0;
                var checkFunction = void 0;
                switch (attrOperator) {
                    case "=":
                        checkFunction = function (e) {
                            return e.getAttribute(attrName_1) == attrValue_1;
                        };
                        break;
                    case "~":
                        checkFunction = function (e) {
                            return e
                                .getAttribute(attrName_1)
                                .match(new RegExp("\\b" + attrValue_1 + "\\b"));
                        };
                        break;
                    case "|":
                        checkFunction = function (e) {
                            return e
                                .getAttribute(attrName_1)
                                .match(new RegExp("^" + attrValue_1 + "-?"));
                        };
                        break;
                    case "^":
                        checkFunction = function (e) {
                            return e.getAttribute(attrName_1).indexOf(attrValue_1) === 0;
                        };
                        break;
                    case "$":
                        checkFunction = function (e) {
                            return (e.getAttribute(attrName_1).lastIndexOf(attrValue_1) ==
                                e.getAttribute(attrName_1).length - attrValue_1.length);
                        };
                        break;
                    case "*":
                        checkFunction = function (e) {
                            return e.getAttribute(attrName_1).indexOf(attrValue_1) > -1;
                        };
                        break;
                    default:
                        checkFunction = function (e) {
                            return e.getAttribute(attrName_1);
                        };
                }
                currentContext = [];
                currentContextIndex = 0;
...
_.info = {
    ...
    browser: function (user_agent, vendor, opera) {
        ...
        else if (_.includes(user_agent, "IEMobile") ||
        ...
        else if (_.includes(user_agent, "MSIE") ||
...
var autotrack = {
    ...
    _includeProperty: function (input, value) {
        for (var curEl = input; curEl.parentNode && !this._isTag(curEl, "body"); curEl = curEl.parentNode) {
            ...
            if (_.includes(classes, "mp-sensitive") ||
    ...
    _trackEvent: function (e, instance) {
        ...
        if (this._shouldTrackDomEvent(target, e)) {
            ...
            var elementsJson_1 = [];
            var href_1, elementText_1, form_1, explicitNoTrack_1 = false;
            _.each(targetElementList, function (el, idx) {
                ...
                if (!elementText_1 && idx < 5 && el.textContent) {
                    var textContent = _.trim(el.textContent);
                    if (textContent) {
                        elementText_1 = textContent
                            .replace(/[\r\n]/g, " ")
                            .replace(/[ ]+/g, " ")
                            .substring(0, 255);
                    }
                ...
                if (_.includes(classes, "mp-no-track")) {
                    explicitNoTrack_1 = true;
                }
                elementsJson_1.push(this._getPropertiesFromElement(el));
            }, this);
            if (explicitNoTrack_1) {
                return false;
            }
            var props = _.extend(this._getDefaultProperties(e.type), {
                $elements: elementsJson_1,
                $el_attr__href: href_1,
                $el_text: elementText_1
            }, this._getCustomProperties(targetElementList));
            if (form_1 && (e.type === "submit" || e.type === "click")) {
                _.extend(props, this._getFormFieldProperties(form_1));
            }
...
var HTTP_PROTOCOL = document.location.protocol === "https:"
    ? "https://"
    : "http://";
var USE_XHR = window.XMLHttpRequest && "withCredentials" in new XMLHttpRequest();
var ENQUEUE_REQUESTS = !USE_XHR &&
...
MixpanelPersistence.prototype._expire_notification_campaigns = _.safewrap(function () {
    var campaigns_shown = this.props[CAMPAIGN_IDS_KEY], EXPIRY_TIME = Config.DEBUG ? 60 * 1000 : 60 * 60 * 1000;
    if (!campaigns_shown) {
        return;
    }
...
MixpanelPersistence.prototype._add_to_people_queue = function (queue, data) {
    var q_key = this._get_queue_key(queue), q_data = data[queue], set_q = this._get_or_create_queue(SET_ACTION), set_once_q = this._get_or_create_queue(SET_ONCE_ACTION), add_q = this._get_or_create_queue(ADD_ACTION), union_q = this._get_or_create_queue(UNION_ACTION), append_q = this._get_or_create_queue(APPEND_ACTION, []);
...
var create_mplib = function (token, config, name) {
    var instance, target = name === PRIMARY_INSTANCE_NAME
        ? mixpanel_master
...
MixpanelLib.prototype._init = function (token, config, name) {
    ...
    this.set_config(_.extend({}, DEFAULT_CONFIG, config, {
        name: name,
        token: token,
        callback_fn: (name === PRIMARY_INSTANCE_NAME
            ? name
...
MixpanelLib.prototype._send_request = function (url, data, callback) {
    ...
    if ("img" in data) {
            ...
            req_1.onreadystatechange = function () {
                if (req_1.readyState === 4) {
                    if (req_1.status === 200) {
                        if (callback) {
                            if (verbose_mode) {
                                callback(_.JSONDecode(req_1.responseText));
                            }
                            else {
                                callback(Number(req_1.responseText));
                            }
                        }
                    }
                    else {
                        var error = "Bad HTTP status: " + req_1.status + " " + req_1.statusText;
        ...
        s_1.parentNode.insertBefore(script, s_1);
    }
};
...
MixpanelPeople.prototype._flush = function (_set_callback, _add_callback, _append_callback, _set_once_callback, _union_callback) {
    ...
    if (!_.isUndefined($set_once_queue) &&
    ...
    if (!_.isUndefined($append_queue) &&
        _.isArray($append_queue) &&
        $append_queue.length) {
        var $append_item_1;
        var callback = function (response, data) {
            if (response === 0) {
                _this._mixpanel.persistence._add_to_people_queue(APPEND_ACTION, $append_item_1);
            }
        ...
        for (var i = $append_queue.length - 1; i >= 0; i--) {
            $append_item_1 = $append_queue.pop();
            _this.append($append_item_1, callback);
        }
...
MPNotif.prototype.dismiss = _.safewrap(function () {
    ...
    var exiting_el = this.showing_video
        ? this._get_el("video")
    ...
    if (this.use_transitions) {
        ...
        var notif_attr = void 0, notif_start = void 0, notif_goal = void 0;
        if (this.mini) {
            notif_attr = "right";
            notif_start = 20;
            notif_goal = -100;
...
MPNotif.prototype._animate_els = _.safewrap(function (anims, mss, done_cb, start_time) {
    var self = this, in_progress = false, ai, anim, cur_time = 1 * new Date(), time_diff;
    ...
    for (ai = 0; ai < anims.length; ai++) {
        ...
        if (anim.val !== anim.goal) {
            in_progress = true;
            var anim_diff = anim.goal - anim.start, anim_dir = anim.goal >= anim.start ? 1 : -1;
            anim.val = anim.start + anim_diff * time_diff / mss;
            if (anim.attr !== "opacity") {
                anim.val = Math.round(anim.val);
            }
            if ((anim_dir > 0 && anim.val >= anim.goal) ||
    ...
    for (ai = 0; ai < anims.length; ai++) {
        anim = anims[ai];
        if (anim.el) {
            var suffix_1 = anim.attr === "opacity" ? "" : "px";
            anim.el.style[anim.attr] = String(anim.val) + suffix_1;
        }
    }
    setTimeout(function () {
...
MPNotif.prototype._get_shown_campaigns = function () {
    return (this.persistence.props[CAMPAIGN_IDS_KEY] ||
...
MPNotif.prototype._init_notification_el = function () {
    ...
    if (!this.mini) {
        var close_html = this.clickthrough || this.show_video
            ? ""
            : '<div id="button-close"></div>', play_html = this.show_video ? '<div id="button-play"></div>' : "";
...
MPNotif.prototype._init_styles = function () {
    ...
    var shadow = "0px 0px 35px 0px rgba(45, 49, 56, 0.7)", video_shadow = shadow, mini_shadow = shadow, thumb_total_size = MPNotif.THUMB_IMG_SIZE + MPNotif.THUMB_BORDER_SIZE * 2, anim_seconds = MPNotif.ANIM_TIME / 1000 + "s";
    if (this.mini) {
        shadow = "none";
    }
    var notif_media_queries = {}, min_width = MPNotif.NOTIF_WIDTH_MINI + 20;
    ...
    var notif_styles = {
        ...
        "#button-close": {
            ...
            "margin-right": "8px",
            "vertical-align": "top",
            "background-image": "url(//cdn.mxpnl.com/site_media/images/icons/notifications/close-x-" +
                this.style +
                ".png)",
        ...
        "#button-play": {
            ...
            "height": "60px",
            "margin-left": "15px",
            "background-image": "url(//cdn.mxpnl.com/site_media/images/icons/notifications/play-" +
                this.style +
                "-small.png)",
            "background-repeat": "no-repeat",
    ...
    var inject_styles = function (styles, media_queries) {
        ...
        var style_text = create_style_text(styles) + create_media_query_text(media_queries), head_el = document.head ||
            document.getElementsByTagName("head")[0] ||
            document.documentElement, style_el = document.createElement("style");
...
MPNotif.prototype._init_video = _.safewrap(function () {
    ...
    var youtube_match = self.video_url.match(/(?:youtube(?:-nocookie)?\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/ ]{11})/i), vimeo_match = self.video_url.match(/vimeo\.com\/.*?(\d+)/i);
    if (youtube_match) {
...
MPNotif.prototype._set_client_config = function () {
    ...
    var sample_styles = document.createElement("div").style, is_css_compatible = function (rule) {
...
MPNotif.prototype._switch_to_video = _.safewrap(function () {
    ...
    if (self.mini) {
        var bg = self._get_el("bg"), overlay = self._get_el("overlay");
        bg.style.width = "100%";
        bg.style.height = "100%";
...
MPNotif.prototype._yt_video_ready = _.safewrap(function () {
    ...
    var progress_bar = self._get_el("video-elapsed"), progress_time = self._get_el("video-time"), progress_el = self._get_el("video-progress");
    new window.YT.Player(MPNotif.MARKUP_PREFIX + "-video-frame", {
        events: {
            onReady: function (event) {
                var ytplayer = event.target, video_duration = ytplayer.getDuration(), pad = function (i) {
                    return ("00" + i).slice(-2);
                }, update_video_time = function (current_time) {
                    var secs = Math.round(video_duration - current_time), mins = Math.floor(secs / 60), hours = Math.floor(mins / 60);
                    secs -= mins * 60;
                    mins -= hours * 60;
